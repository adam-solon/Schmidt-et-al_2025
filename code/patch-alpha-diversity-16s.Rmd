...
### Input files  
Phyloseq objects from script 'patch-normalization-transformation-16s.Rmd' 

```{r import phyloseq object}
#input phyloseq object with raw, transformed sequence counts
phy.2 <- readRDS("F:/Projects/Sunspears/Studies/Patch_Schmidt-et-al/analyses/amplicons/16s/norm_transform/objects/phy.2.rds")
...
for(sam in samples) {
  ...
  for(i in 1:1000) {
    ...
    suppressWarnings({
      # Rarefy data
      rarefaction <- rrarefy(sample_data, raremax)
...
#import R object-
div.res <- readRDS("F:/Projects/Sunspears/Studies/Patch_Schmidt-et-al/analyses/amplicons/16s/alpha_diversity/objects/div.res.rds")
...
p.rich <- ggplot(avg.res.2, aes(x= Patch, y= Richness)) +
  ...
  scale_y_continuous(expand = c(0.0, 0.0), limits = c(0, 255), breaks = c(0, 50, 100, 150, 200, 250)) +
  ...
  annotate("text", x = 1, y = 245, label = "a", size = 6) +
  annotate("text", x = 2, y = 245, label = "b", size = 6) +
  theme_bw() + 
...
# Create scatterplot
p.shan <- ggplot(avg.res.2, aes(x= Patch, y= Shannon)) +
  geom_boxplot(aes(fill = Patch)) +
  geom_jitter(size= 4, shape= 1, color = "black", width = 0.08) +
  scale_y_continuous(expand = c(0.0, 0.0), limits = c(0, 5.1), breaks = c(0, 1, 2, 3, 4, 5)) +
  scale_x_discrete(labels = c("colony" = "Colonies", "bare" = "Bare Sediment"), expand = c(0.4, 0.1), drop = FALSE) +
  scale_fill_manual(values= c("#018571", "tan")) + # Patch_type blind friendly palette - RPatch_typeBrewer
  guides(fill = "none") +
  labs(y = "Shannon Diversity") +
  annotate("text", x = 1, y = 4.9, label = "a", size = 6) +
  annotate("text", x = 2, y = 4.9, label = "b", size = 6) +
  theme_bw() + 
  theme(axis.title.y = element_text(size = 20, face = "bold"), 
        axis.title.x = element_blank(),
        axis.text.x = element_text(size = 18, face = "bold"),
...
p.even <- ggplot(avg.res.2, aes(x= Patch, y= Evenness)) +
  ...
  #annotate("text", x = 2, y = 0.95, label = "a", size = 6) +
  theme_bw() + 
  theme(axis.title.y = element_text(size = 20, face = "bold"), 
        axis.title.x = element_blank(),
        axis.text.x = element_text(size = 18, face = "bold"),
...
#shannon scatterplot
saveRDS(p.shan, paste0(objects.fp, "/p.shan.rds"))
...
setwd("F:/Projects/Sunspears/Studies/Patch_Schmidt-et-al/analyses/amplicons/16s/alpha_diversity/output/stats/Patch_type")
getwd()
...
write.csv(avg.res.3, "patch-alpha-diversity-16s-by-sample.csv", row.names = FALSE)

#save summary statistics
write.csv(avg.soil.age.res.rich, "Richness_16s_summary_stats.csv", row.names = FALSE)

#save summary statistics
write.csv(avg.soil.age.res.shan, "Shannon_16s_summary_stats.csv", row.names = FALSE)

#save summary statistics
write.csv(avg.soil.age.res.even, "Evenness_16s_summary_stats.csv", row.names = FALSE)

#save pairwise comparisons
# Save richness the results as a CSV file
capture.output(print(t.test.rich), file = "Richness_16s_tTest.txt")

#shannon
capture.output(print(t.test.shan), file = "Shannon_16s_tTest.txt")

#Evenness
capture.output(print(t.test.even), file = "Evenness_16s_tTest.txt")

```

```{r save rich figures, include = FALSE}
#check and set working directory
setwd("F:/Projects/Sunspears/Studies/Patch_Schmidt-et-al/analyses/amplicons/16s/alpha_diversity/output/plots/Patch_type/")
getwd()

#richness
#save plot as pdf
pdf("richness-16s-rarefied-4131.pdf",         # File name
...
##save pdf
cairo_pdf("richness-16s-rarefied-4131-II.pdf", width = 11, height = 8.5, family = "Helvetica")
...
#Save as jpg
jpeg(filename = "richness-16s-rarefied-4131.jpeg", 
     width = 1000, 
     height = 600, 
     units = "px", 
     pointsize = 12, 
...
#save as .svg file
ggsave("richness-16s-rarefied-4131.svg", plot = p.rich, device = "svg", width = 11, height = 8.5, units = "in")
...
#shannon
#save plot as pdf
pdf("shannon-16s-rarefied-4131.pdf",         # File name
    width =11, height = 8.5, # Width and height in inches
    bg = "white",          # Background Patch_type
    colormodel = "cmyk",  # Patch_type model (cmyk is required for most publications)
    paper = "USr")          # Paper size-- portrait: a4", "letter", "legal" (or "us") | landscape: "a4r" and "USr"
...
##save pdf
cairo_pdf("shannon-16s-rarefied-4131-II.pdf", width = 11, height = 8.5, family = "Helvetica")
...
#Save as jpg
jpeg(filename = "shannon-16s-rarefied-4131.jpeg", 
     width = 1000, 
     height = 600, 
     units = "px", 
     pointsize = 12, 
...
#save as .svg file
ggsave("shannon-16s-rarefied-4131.svg", plot = p.shan, device = "svg", width = 11, height = 8.5, units = "in")
...
#evenness
#save plot as pdf
pdf("evenness-16s-rarefied-4131.pdf",         # File name
    width =11, height = 8.5, # Width and height in inches
    bg = "white",          # Background Patch_type
    colormodel = "cmyk",  # Patch_type model (cmyk is required for most publications)
    paper = "USr")          # Paper size-- portrait: a4", "letter", "legal" (or "us") | landscape: "a4r" and "USr"
...
##save pdf
cairo_pdf("evenness-16s-rarefied-4131-II.pdf", width = 11, height = 8.5, family = "Helvetica")
...
#Save as jpg
jpeg(filename = "evenness-16s-rarefied-4131.jpeg", 
     width = 1000, 
     height = 600, 
     units = "px", 
     pointsize = 12, 
...
#save as .svg file
ggsave("evenness-16s-rarefied-4131.svg", plot = p.even, device = "svg", width = 11, height = 8.5, units = "in")
