...
subtitle: "  \n Beta Diversity - 18S SSU rRNA gene"
author: "Adam J. Solon"
date: "`r Sys.Date()`"
...
---
# Script Summary  
This script analyzes the eukaryotic (18S SSU rRNA gene) beta diversity of samples from soils collected in bare soils and biocrust patches near the terminus of the Midtre Lovenbreen glacier.  The overarching question: 

How different are eukaryotic communities between green or black tinted biocrusts and between those biocrusts and the surrounding bare ground.
...
analysis2 <- "/BD_beta_diversity/18S"
...
1. ASV table filtered and subsest through r script - 'MCosms_II_seq_processing_18s.r'
2. Mapping file subset through r script - 'MCosms_II_seq_processing_18s.r'
  
```{r input data files, include = FALSE}
# input data files
# ASV Table (from DADA2)
asvTable.fp <- paste0(input.fp, "/ASVTable_18S_wTax_patch.txt") 
...
t <- a %>% select(ASV_ID, Domain, SuperGroup, CladeH, CladeM, CladeL, CladeLL, GenusSpecies)
...
\newpage   
### Plot
```{r display plot 1,  include = TRUE, fig.cap = "PCA of 18S marker gene from Patches of biocrust and bare soils of the glacial forefield of Midtre Lovenbreen on Svalbard. Each sample is marked Sample ID."}
#display plot
pca.plot.1
...
pca.fig.1 <- ggplot(df1, aes(PC1, PC2, color = type))+ 
  ...
  annotate("text", x = .25, y = -1.15, label = "Bare", size = 10) +
  annotate("text", x = -1.25, y = 1.30, label = "Black", size = 10) +
  annotate("text", x = 0.9, y = 0.75, label = "Green", size = 10) +
  theme_classic() +
...
\newpage   
### Figure
```{r display figure 1, include = TRUE, fig.cap = "PCA of 18S marker gene from Patches of biocrust and bare soils of the glacial forefield of Midtre Lovenbreen on Svalbard Each sample is marked a cricle and coloread according to patch type."}
#display plot
pca.fig.1
...
\newpage 
### Figure
```{r display dispersion figure 1, include = TRUE, fig.cap= "Boxplot of the average distance to spatial median of each sample group."}
#display
bd.1.box
...
setwd("~/Desktop/Svalbard/Patches/analyses/dna_seq/amplicons/BD_beta_diversity/18S")
getwd()
...
#save plot as pdf
pdf("output/patch_18s_pca.pdf",         # File name
...
#Save as jpg
jpeg(filename = "output/patch_18s_pca.jpeg", 
     width = 600, 
     height = 600, 
     units = "px", 
     pointsize = 12, 
...
#save plot as pdf
pdf("output/patch_18s_bd.1.box.pdf",         # File name
    width =11, height = 8.5, # Width and height in inches
    bg = "white",          # Background color
    colormodel = "cmyk",  # Color model (cmyk is required for most publications)
    paper = "USr")          # Paper size-- portrait: a4", "letter", "legal" (or "us") | landscape: "a4r" and "USr"
...
#Save as jpg
jpeg(filename = "output/patch_18s_bd.1.box.jpeg", 
     width = 800, 
     height = 600, 
     units = "px", 
     pointsize = 12, 
...
writeLines(out.1, con = file("patches_18s_pmanova_pw_1.txt"))

#combine summary outputs as object 
out.2 <- capture.output(print(bd.1), print(bd.perm.1))
#save homogeneity of dispersion stats
writeLines(out.2, con = file("patches_18s_betadisper_1.txt"))
