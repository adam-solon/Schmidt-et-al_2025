...
subtitle: "Normalization & Transformation - 16S SSU rRNA gene"
author: "Adam J. Solon"
date: "`r Sys.Date()`"
...
# Script Summary  
This script applies standard normalization and transformation of sequencing data to account for differences in library size (i.e., read depth) that confound downstream analyses.
...
```{r set path for processed files}
#directory.1
###################################################
...
input.fp <- paste0(analysis.1.fp, "/objects")
if (!dir.exists(input.fp)) dir.create(input.1.1.fp, recursive = TRUE)

# Create sub-directory analysis 2
###################################################
analysis.2.fp <- paste0(directory.1.fp, analysis.2)
if (!dir.exists(analysis.2.fp)) dir.create(analysis.2.fp, recursive = TRUE)

# Create sub-directories for analysis 
###################################################

# Create directory for 'within' pipeline R objects 
objects.fp <- paste0(analysis.2.fp, "/objects")
if (!dir.exists(objects.fp)) dir.create(objects.fp, recursive = TRUE)

# Create directory for 'after' pipeline outputs 
output.fp <- paste0(analysis.2.fp, "/output")
...
1. ASV table filtered and subsest through r script - 'patch-seq-processing-dna-16s.Rmd'
2. Mapping file subset through r script - 'patch-seq-processing-dna-16s.Rmd' 

```{r input data files, include = FALSE}
# input data files
# ASV Table (from DADA2)
asvTable.fp <- paste0(input.fp, "/ASV.table.13.wTax.filtered.rds") 

# Mapping File (metadata relevant for study samples)
mappingFile.fp <- paste0(input.fp, "/m.2.rds") 

#input 16S ASV table w/ taxonomy
#a <- read.table(asvTable.fp, header = T, sep = "\t")

a <- readRDS(asvTable.fp)
...
#create a dataframe with only soil age and sample ID
m.1 <- m %>% 
  dplyr::select(SampleID, Patch_type, Color) %>%
  dplyr::mutate(Patch_type = as.factor(Patch_type)) %>%
...
phy.2 <- tax_glom(phy.1, taxrank = "Genus")

#save R object
saveRDS(phy.2, paste0(objects.fp, "/phy.2.rds"))
...
#rename 'r' column as 'richness_16s'
df.u.2 <- df.u.2 %>% rename(raw_richness_16S = h)
...
phy.r <- rarefy_even_depth(phy, 
                           sample.size = 4131, #no. of reads for cut-off, number is generated from 'raremax' object above
...
#agglomerate to the genera level (to account for short read taxonomic assignment most reliable down to genus, not below)
phy.r.2 <- tax_glom(phy.r.1, taxrank = "Genus")

#save R object
saveRDS(phy.r.2, paste0(objects.fp, "/phy.r.2.rds"))

```

```{r import phyloseq object rarefy}
#import R object
phy.r.2 <- readRDS("F:/Projects/Sunspears/Studies/Patch_Schmidt-et-al/analyses/amplicons/16s/norm_transform/objects/phy.r.2.rds")
...
#rename 'r' column as 'richness_16s'
df.r.2 <- df.r.2 %>% rename(rarefied_richness_16S = r)

#reorganize columns
df.r.3 <- df.r.2 #[, c(1:23, 50, 24:49)]

#add rarefied and raw richness together
df.u.3$rarefied_richness_16S <- df.r.3$rarefied_richness_16S
...
```{r save files}
# save as an R file 
saveRDS(df.u.3, paste0(output.fp, "/m.rich.rds"))

#save as .txt file 
write.table(df.u.3, file = paste0(output.fp, "/patch-richness.txt"), 
