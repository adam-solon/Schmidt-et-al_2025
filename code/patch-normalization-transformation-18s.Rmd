...
subtitle: "Normalization & Transformation - 18S SSU rRNA gene"
author: "Adam J. Solon"
date: "`r Sys.Date()`"
...
asvTable.fp <- paste0(input.fp, "/ASV.table.12.wTax.filtered.rds") 

# Mapping File (metadata relevant for study samples)
mappingFile.fp <- paste0(input.fp, "/m.1.rds") 
...
t <- a %>% dplyr::select(ASV_ID, Domain, SuperGroup, CladeH, CladeM, CladeL, CladeLL, CladeLLL)
...
phy.2 <- tax_glom(phy.1, taxrank = "CladeLLL")
...
#rarefy
a.rare <- rarefy(a.u.t, 550)
  #rarefy(a.u.t, raremax)
...
phy.r <- rarefy_even_depth(phy, 
                           sample.size = 550, #no. of reads for cut-off, number is generated from 'raremax' object above
...
#agglomerate to the genera level (to account for short read taxonomic assignment most reliable down to genus, not below)
phy.r.2 <- tax_glom(phy.r.1, taxrank = "CladeLLL")
...
#import R object
phy.r.2 <- readRDS("F:/Projects/Sunspears/Studies/Patch_Schmidt-et-al/analyses/amplicons/18s/norm_transform/objects/phy.r.2.rds")
...
df.b.1 <-  df.u.3 %>%
  left_join(df.r.3 %>% select(SampleID, rarefied_richness_18S), by = "SampleID")

#remove superfluous column and rename existing rarefied richness column
#df.b.1 <- df.b.1 %>% rename(rarefied_richness_18S = rarefied_richness_18S.y)
#df.b.1$rarefied_richness_18S.x <- NULL
...
#save as .txt file 
write.table(df.u.3, file = paste0(output.fp, "/patch-richness-18s.txt"), 
