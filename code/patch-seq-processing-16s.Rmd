...
directory.2 <- "/analyses/amplicons/16s"
...
asvTable.fp <- paste0(directory.1.fp, "/16Sseqtab_wtax_mctoolsr.txt") 

# Mapping File (metadata relevant for study samples)
mappingFile.fp <- paste0(directory.1.fp, "/patch_mapfile_16s.txt") 
...
#rename ESV_ID
a <- dplyr::rename(a, ASV_ID = ESV_ID)
...
t.1 <- stringr::str_split_fixed(t$taxonomy, ";", 6)

#rename columns w/ taxonomic ranks for 16S data
colnames(t.1)[1:6] <- c("Domain", "Phylum", "Class", "Order", "Family", "Genus")
...
#remove ASVs that are eukaryotes (because these are 16S-amplified libraries NOT 16S) 
a.10 <- a.9 %>% filter(Domain != c("Eukaryota")) 

#r3emove ASVs that are unassigned anything below Domain level
a.11 <- a.10 %>% filter(Phylum != "NA")

#remove chloroplast that are identified as cyanobacteria
a.12 <- a.11 %>% filter(Order != "Chloroplast")

#remove mitochondria that are identified as alphaproteobacteria
a.13 <- a.12 %>% filter(Family != "Mitochondria")

# save as an R file
saveRDS(a.13, paste0(objects.fp, "/ASV.table.13.wTax.filtered.rds"))

#save as .txt file
write.table(a.13, file = paste0(output.fp, "/asvTab_16S_wTax_filtered.txt"), 
            sep = "\t", row.names = FALSE, col.names = TRUE)
...
```{r}
#row names as SampleID column
rownames(a.13) <- a.13$ASV_ID

#remove 'SampleID' column
a.13$ASV_ID <- NULL

#remove taxonomy columns
a.14 <- subset (a.13, select = -c(Domain:Genus))

#transpose
a.14 <- as.data.frame(t(a.14))
```
...
```{r}
# create row sum column
a.lib.size.filt <- a.14 %>% mutate(library_size = rowSums(.))
...
#move row names of samples into 1st column as 'SampleID' to match 1st column of mapping file
a.16 <- tibble::rownames_to_column(a.15, "ASV_ID")

#add taxonomy columns back into ASV table with a Left Join using inner_join function from dplyr
a.16.T <- a.16 %>% inner_join(t.2, by= "ASV_ID")

# save asv table w/ tax as .rds
saveRDS(a.16.T, paste0(objects.fp, "/asv.wTax.patch.rds"))

#save asv table w/ tax as .txt file 
write.table(a.16.T, file = paste0(input.fp, "/ASVtable_16S_wTax_patch.txt"), 
            sep = "\t", row.names = FALSE, col.names = TRUE)
...
saveRDS(m, paste0(objects.fp, "/m.2.rds"))

#save non-blanks mapping file subset as .txt file 
write.table(m, file = paste0(output.fp, "/map_file_patch.txt"), 
            sep = "\t", row.names = FALSE, col.names = TRUE)
